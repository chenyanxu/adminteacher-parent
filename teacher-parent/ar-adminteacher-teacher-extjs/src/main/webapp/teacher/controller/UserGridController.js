 ...
 * @version 1.0.0
 */
Ext.define('kalix.adminteacher.teacher.controller.UserGridController', {
  extend: 'kalix.controller.BaseGridController',
  alias: 'controller.teacherUserGridController',
  requires: ['kalix.Notify'],
  onEdit: function (grid, rowIndex, colIndex) {
    var oldView = this.cfgForm;
    this.cfgForm = 'kalix.adminteacher.teacher.view.UserEditWindow';
  ...
  onKey: function (grid, rowIndex, colIndex) {
    ...
    var view = Ext.create('kalix.adminteacher.teacher.view.UserKeyWindow');
  ...
  onAuthorization: function (grid, rowIndex, colIndex) {
    /*var authorizationWindow = Ext.create('kalix.adminteacher.common.components.AuthorizationViewWindow');
    var rec = grid.getStore().getAt(rowIndex);
    authorizationWindow.roleId = rec.data.id;
    authorizationWindow.authorizationUrl = CONFIG.restRoot + '/camel/rest/teachers/' + rec.data.id + '/authorizations';
    ...
    store.setProxy({
      type: 'ajax',
      url: CONFIG.restRoot + '/camel/rest/teachers/' + rec.data.id + '/authorizations'
    });
